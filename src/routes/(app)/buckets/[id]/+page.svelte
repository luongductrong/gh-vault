<script lang="ts">
	...
	const bucketQuery = createQuery(() => ({
		...
		queryFn: () => fetchApi<any>(`/buckets/${bucketId}`)
	}));

	// Fetch files
	const filesQuery = createQuery(() => ({
		queryKey: ['buckets', bucketId, 'files'],
		queryFn: () => fetchApi<{ data: any[]; total: number }>(`/buckets/${bucketId}/files?limit=50`)
	}));
...
<div class="container mx-auto max-w-7xl p-4 md:p-6 lg:p-8 space-y-8">
	<!-- Header & Stats -->
	...
	{:else if bucketQuery.isError}
		<Card.Root class="border-destructive/50 bg-destructive/10">
			<Card.Content class="p-6">
				<p class="text-destructive font-medium">Failed to load vault details</p>
			</Card.Content>
		</Card.Root>
	{:else if bucketQuery.data}
		{@const b = bucketQuery.data}
		<div class="flex flex-col gap-6 md:flex-row md:items-start md:justify-between border-b border-border pb-6">
			<div>
				<div class="flex items-center gap-3 mb-2">
					<a href="/" class="text-muted-foreground hover:text-foreground transition-colors">
						<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-arrow-left"><line x1="19" x2="5" y1="12" y2="12"/><polyline points="12 19 5 12 12 5"/></svg>
					</a>
					<h1 class="text-3xl font-bold tracking-tight">{b.displayName || b.githubRepoName}</h1>
				</div>
				<div class="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-muted-foreground ml-8">
			...
			<div class="flex items-center">
				...
				>
					{#if uploadMutation.isPending}
						<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-loader-2 mr-2 animate-spin"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
	...
	<div class="pt-2">
		...
		{:else if filesQuery.isError}
			<div class="flex items-center justify-center p-12 text-destructive border border-destructive/20 rounded-lg">
				Failed to load files: {filesQuery.error.message}
			</div>
		{:else if filesQuery.data?.data.length === 0}
			<div class="flex flex-col items-center justify-center p-20 text-center border border-dashed border-border rounded-lg text-muted-foreground bg-muted/20">
				<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-image-off mb-4 opacity-50"><line x1="2" x2="22" y1="2" y2="22"/><path d="M10.41 10.41a2 2 0 1 1-2.83-2.83"/><line x1="13.5" x2="6" y1="13.5" y2="21"/><line x1="18" x2="21" y1="12" y2="15"/><path d="M3.59 3.59A1.99 1.99 0 0 0 3 5v14a2 2 0 0 0 2 2h14c.55 0 1.05-.22 1.41-.59"/><path d="M21 15V5a2 2 0 0 0-2-2H9"/></svg>
				<p>No files uploaded yet.</p>
			</div>
		{:else}
			<div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
				{#each filesQuery.data?.data || [] as file}
					<Card.Root class="group relative overflow-hidden flex flex-col">
						<div class="aspect-square bg-muted/30 relative overflow-hidden">
							...
							<!-- Hover Overlay -->
							<div class="absolute inset-0 bg-background/80 opacity-0 transition-opacity flex items-center justify-center gap-2 group-hover:opacity-100 backdrop-blur-sm">
								<Button size="icon" variant="secondary" onclick={() => copyToClipboard(file.cdnUrl)} title="Copy CDN Link">
									<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-copy"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>
								</Button>
								<Button size="icon" variant="secondary" href={file.cdnUrl} target="_blank" rel="noopener noreferrer" title="Open original">
						...
						<div class="p-2 border-t border-border flex flex-col">
							<span class="text-xs font-medium truncate" title={file.originalName}>{file.originalName}</span>
